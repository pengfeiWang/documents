...
(function(expose) {
 ...
 /**
  ...
  Markdown.prototype.split_blocks = function splitBlocks( input ) {
    ...
    // console.log( blocks.toString(), '======999')
    // console.log( blocks.length, '======999')
  ...
  /** 
   *降价＃processBlock（块，下一个） - >未定义|[JsonML，...] 
   * - 块（字符串）：要处理的块 
   * - 下一个（阵列）：以下块 
   * 
   *处理`block`并返回一个表示'block` JsonML节点的数组。 
   * 
   *它通过询问每一个块级功能的话来处理这是否 
   *块，直到能及。成功的处理是通过返回一个表示 
   *阵列（零个或多个JsonML节点），通过一个假值失败。 
   * 
   *处理器模块负责调用[[降价＃processInline] 
   *自己是合适的。 
   * 
   *如果块被分割不正确或相邻块需要你崩溃 
   *可以使用移位调整`next`到位/接头等 
   * 
   *如果有任何的默认行为是不正确的话，你可以 
   *定义上，将被调用来处理方言`__call__`方法 
   *块处理。 
   */
  ...
  Markdown.prototype.toTree = function toTree( source, custom_root ) {
    var blocks = source instanceof Array ? source : /*[source];*/ this.split_blocks( source );
    var t = [];
    ...
    try {
      ...
      while ( blocks.length ) {
        var s = blocks.shift();
        var b = this.processBlock( s, blocks );
  ...
  Markdown.toHTML = function toHTML( source , dialect , options ) {
    ...
    var html = this.renderJsonML( input )
    return html;
  };
  ...
  function render_tree( jsonml ) {
    ...
    // if( tag === 'pre') {
    //   if( jsonml[0][1] ) {
    //     var s = /css|js|html|php$/.exec( jsonml[0][1] );
    //     if( s ) {
    //       var tag_attrsClass = 'class="' + s[0] + '"';
    //     }
    //     jsonml[0][1] = jsonml[0][1].replace(/^\n/,'').replace(/\n(css|html|php|js)/,'')
    //   }
    // }

    if( tag == 'code' ) {
      // console.log(jsonml[0], jsonml[1])
      // var tag_attrsClass = 'class="css"';

      if( jsonml[0] ) {
        var s = /css|js|html|php$/.exec( jsonml[0] );
        if( s ) {
          var tag_attrsClass = 'class="' + s[0] + '"';
        }
        jsonml[0] = jsonml[0].replace(/^\n/,'').replace(/\n(css|html|php|js)/,'')
      }
    }
    while ( jsonml.length )
      content.push( render_tree( jsonml.shift() ) );
    ...
    if ( tag === "img" || tag === "br" || tag === "hr" )
      ...
      return "<"+ tag + tag_attrs + (tag_attrsClass?' '+tag_attrsClass:'')+ ">" + content.join( "" ) + "</" + tag + ">";
...
   **/
  var Gruber = {
    block: {
      ...
      newCode: function ( block, next ) {
        var ret = [],
            re = /^(?:`{3})(?:js|html|css|php)([\s\S]+)`{3}/,
            re2 = /^(?:`{3})(js|html|css|php)([\s\S]+)`{3}/;
        if ( !block.match( re ) ) 
          return undefined; 
            /*
    ...
    inline: {
      ...
      // chunks of plain text blocks don't include them \ [` * _ { } < > [ ] ( ) # + . ! - ]
      "]": function () {},
      "}": function () {},
